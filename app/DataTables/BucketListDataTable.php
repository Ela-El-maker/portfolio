...
class BucketListDataTable extends DataTable
{
    /**
        ...
        return (new EloquentDataTable($query))
            ...
            ->addColumn('show_toggle', function ($query) {
                ...
                <label class="custom-switch mt-0">
                    <input type="checkbox" class="custom-switch-input bucketList_status" data-id="' . $query->id . '" ' . $checked . '>
                    <span class="custom-switch-indicator"></span>
                </label>';
            })

            ->addColumn('action', function ($query) {
                return '<a href="' . route('admin.bucket-list.edit', $query->id) . '" class="btn btn-primary"><i class="fas fa-edit"></i></a> <a href="' . route('admin.bucket-list.destroy', $query->id) . '" class="btn btn-danger delete-item"><i class="fas fa-trash-alt"></i></a>';
            })
            ->setRowId('id')
     ...
     */
    public function query(BucketList $model): QueryBuilder
    {
    ...
    {
        return 'BucketList_' . date('YmdHis');
    }
}
